...
void w24fda_collect_info(char* date, char* file_info, int new_socket) {
    ...
    char* tar_filename1 = "w24project/temp.tar.gz";
    struct tm tm_date;
...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(buffer, "true") == 0) {
            clientCountTotal += 1;
            if (clientCountTotal <= 3) {
                send(client_socket, "7000", 4, 0);
            }
            else if (clientCountTotal <= 6) {
                send(client_socket, "7001", 4, 0);
            }
            else if (clientCountTotal <= 9) {
                send(client_socket, "7002", 4, 0);
            }
            else {
                if (clientCountTotal % 3 == 1) {
                    send(client_socket, "7000", 4, 0);
                }
                else if (clientCountTotal % 3 == 2) {
                    send(client_socket, "7001", 4, 0);
                }
                else if (clientCountTotal % 3 == 0) {
                    send(client_socket, "7002", 4, 0);
                }
            }
            close(client_socket);
        ...
        printf("client connected to server1\n");
        if ((pid = fork()) == 0) {
            // Child process
